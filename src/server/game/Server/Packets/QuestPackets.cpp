...
{
    TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "QueryQuestInfoResponse() >> Sending quest info packet containing level %u, min level %u and max scaled level %u for quest id %u with sort id %u.", Info.QuestLevel, Info.QuestMinLevel, Info.QuestMaxScalingLevel, Info.QuestID, Info.QuestSortID);
