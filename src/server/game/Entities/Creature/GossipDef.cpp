...
void PlayerMenu::SendQuestGiverQuestList(uint32 BroadcastTextID, ObjectGuid npcGUID)
{
    ...
    for (uint32 i = 0; i < _questMenu.GetMenuItemCount(); ++i)
    {
        ...
        if (Quest const* quest = sQuestDataStore->GetQuestTemplate(questMenuItem.QuestId))
        {
            ...
            questList.GossipTexts.emplace_back(questMenuItem.QuestId, questMenuItem.QuestIcon, quest->GetScaledQuestLevel(player->getLevel(), player->GetZoneId()), quest->MaxScalingLevel, quest->Flags, quest->FlagsEx, quest->IsRepeatable(), title);
        }
    }
