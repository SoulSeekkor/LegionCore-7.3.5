...
void WorldSession::HandleSendMail(WorldPackets::Mail::SendMail& packet)
{
    ...
    {
        TC_LOG_INFO(LOG_FILTER_NETWORKIO, "Player %u is sending mail to %s (GUID: Non-existent!) with subject %s and body %s includes %u items, " UI64FMTD " copper and " UI64FMTD " COD copper with stationery = %u",
...
void WorldSession::HandleGetMailList(WorldPackets::Mail::MailGetList& packet)
{
    ...
    // recalculate m_nextMailDeliverTime and unReadMails
